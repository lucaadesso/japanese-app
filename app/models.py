"""
models.py – SQLAlchemy ORM models for the Japanese learning app.
Tables:
  - User       : authenticated Google users
  - Card       : flash-cards (hiragana, katakana, vocab, …)
...
class Card(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    # Didactic phase: hiragana | katakana | vocab | kanji | phrase
    phase = Column(String, nullable=False, index=True)
    # Group within phase: vowels | ka | sa | ta | na | ha | ma | ya | ra | wa | n
    ...
    romanji = Column(String, nullable=True)         # romanisation
